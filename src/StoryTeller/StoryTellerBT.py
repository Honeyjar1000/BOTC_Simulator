...
class StoryTellerBT():
    ...
    def __init__(self, players=None, scipt=None, player_count=None):
        
        # Create BB
        self.BB = StoryTellerBB(players, scipt, player_count)
        ...
        # First Night
        #############################
        node_condition_first_night = NodeST_ConditionFirstNight("Check First Night Condition", black_board=self.BB)
        node_first_night_minion_info = NodeST_MinionFirstNightInfo("First Night Minion Info", black_board=self.BB)
        node_first_night_demon_info = NodeST_DemonFirstNightInfo("First Night Demon Info", black_board=self.BB)

        # Other info

        node_end_first_night = NodeST_EndFirstNight("End First Night", black_board=self.BB)
        ...
        # Other Night
        #############################
        node_condition_other_night = NodeST_ConditionOtherNight("Check Other Night Condition", black_board=self.BB)
        node_end_other_night = NodeST_EndOtherNight("End Other Night", black_board=self.BB)
        node_nighttime.add_children([node_condition_other_night, node_end_other_night])

        #############################
        # Day Time
        #############################
        node_condition_day = NodeST_ConditionDay("Check Day Condition", black_board=self.BB)
        node_call_townsquare = NodeST_CallTownSquare("Call Townsquare", black_board=self.BB)
        node_daytime.add_children([node_condition_day, node_call_townsquare])

        #############################
        # Town Square
        #############################
        node_condition_day = NodeST_ConditionTownSquare("Check TownSquare Condition", black_board=self.BB)
        node_call_townsquare = NodeST_EndDay("End Day", black_board=self.BB)
