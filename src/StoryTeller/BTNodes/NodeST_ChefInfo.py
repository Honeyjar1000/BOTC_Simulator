...
class NodeST_ChefInfo(py_trees.behaviour.Behaviour):
    ...
    def update(self):
        ...
        b_in_play, chef_player = CheckIfCharacterInPlay(Characters.CHEF, self.black_board.players)
        if b_in_play:
            action = A_ChefInfo(self.black_board.players)
            chef_player.WakeAtNight(action)
