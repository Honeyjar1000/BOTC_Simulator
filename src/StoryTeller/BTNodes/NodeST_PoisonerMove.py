...
class NodeST_PoisonerMove(py_trees.behaviour.Behaviour):
    ...
    def update(self):
        ...
        b_in_play, poisoner_player = CheckIfCharacterInPlay(Characters.POISONER, self.black_board.players)
        if b_in_play:
            action = A_PoisonerMove(self.black_board.players)
            poisoner_player.WakeAtNight(action)
