...
RADIUS = 200  # Radius for spacing between players
NUM_PLAYERS = 10  # Number of players
...
central_entity_image = pygame.transform.scale(central_entity_image, (100, 100))  # Resize to fit well in the center
...
class Player:
    ...
    def update_position(self, num_players, room_center):
        """ Update player's position based on the number of players and room center """
        angle_offset = num_players * (2 * math.pi / NUM_PLAYERS)
        self.x = room_center[0] + RADIUS * math.cos(angle_offset)
        self.y = room_center[1] + RADIUS * math.sin(angle_offset)
...
while running:
    ...
    for i, player in enumerate(players):
        player.update_position(i, ROOM_CENTER)  # Update the player's position based on the current angle
        # Now the player's position is correctly adjusted relative to the room center
        # Instead of subtracting 25, try this and adjust based on image dimensions
        screen.blit(player_image, (player.x - player_image.get_width() // 2, player.y - player_image.get_height() // 2))
